...
const CARDS = {
  ...
  mora: {
    ...
    desc: '+75 HP, +5 ATK. Khi bị đánh có 80% hồi lại 50% sát thương nhận vào (cứ 2 lần +2 ATK, max +96).',
  ...
  atula: {
    ...
    desc: '+75 HP, +5 ATK. Khi bị đánh có 35% tích 100% giá trị vào Huyết Hồn để kết liễu mục tiêu.',
    atkBonus: 5, hpBonus: 75, defBonus: 0
  },
  kolos: {
    id: 'kolos',
    ...
    desc: '+75 HP, +5 ATK. Khi chịu sát thương có 45% khóa toàn bộ thẻ nội tại đối phương trong 2s & hút 30 HP.',
    atkBonus: 5, hpBonus: 75, defBonus: 0
  },
  ayanokouji: {
    id: 'ayanokouji',
  ...
  bles: {
    ...
    desc: '+75 HP, +5 ATK. Cứ mỗi 3s tung Trảm Sát gây 15% ATK và tự tăng 30 ATK vĩnh viễn (max 15 tầng).',
    atkBonus: 5, hpBonus: 75, defBonus: 0
  }
};
...
function executeBlesUltimate() {
  ...
  if ((enemy.blesStacks || 0) < 15) {
    ...
    showSkillBanner('enemy', `⚔️ +30 ATK (${enemy.blesStacks}/15 TẦNG)`, '#ef4444');
...
function strikeOnce(atkObj, defObj, cardIds, atkType, defType, atkName, logCls) {
  ...
  // ATULA: 35% TỈ LỆ TÍCH 100% GIÁ TRỊ VÀO HUYẾT HỒN
  if (defCardIds.includes('atula') && Math.random() < 0.35) {
    let soulAdd = raw;
  ...
  if (defCardIds.includes('kolos') && Math.random() < 0.45) {
